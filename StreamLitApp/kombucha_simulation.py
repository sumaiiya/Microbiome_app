...
sys.path.insert(0, os.path.join(
    BASE_DIR, 'simulation_envs', 'scripts', 'core'))
...
try:
    ...
    if os.path.exists(os.path.join(DATABASEFOLDER, DATABASENAME)):
        ...
        print(
            f"Warning: Database file not found at {os.path.join(DATABASEFOLDER, DATABASENAME)}")
...
def getpH(metabolome_concentration):
    """Add a pH function"""
    pH = min(
        10, max(3, -0.0491*metabolome_concentration[ACETATE_IDX] + 6.6369))
    return pH
...
def getReactor(simulTime=1, SimulSteps=100, dilution=0.0, volume=100):
    ...
    if not ACTUAL_DB_AVAILABLE or DB is None:
        raise RuntimeError(
            "Database or classes not available. Please check your paths and database setup.")

    metabolome = createMetabolome(DB, mediaName='kombucha_media', pHFunc=getpH)
    metabolome_feed = createMetabolome(
        DB, mediaName='kombucha_media', pHFunc=getpH)
    ...
    dilution_param = int(dilution) if isinstance(
        dilution, float) and dilution.is_integer() else dilution

    pulse = Pulse(metabolome_feed, microbiome_feed, 0, simulTime,
                  SimulSteps, 0, 0, dilution_param, dilution_param)
...
class KombuchaGym(gym.Env):
    ...
    def __init__(self,
        ...
        if not ACTUAL_DB_AVAILABLE or DB is None:
            raise RuntimeError(
                "Cannot create KombuchaGym: Database or classes not available")

        self.metabolome_feed = createMetabolome(
            DB, mediaName='kombucha_media', pHFunc=getpH)
        ...
        self.reactor = getReactor(
            self.simulTime, self.SimulSteps, self.dilution, self.volume)
    ...
    def reset(self, seed=None):
        ...
        self.reactor = getReactor(
            simulTime=1, SimulSteps=100, dilution=0.0, volume=100)
        self.reactor.simulate()
        self.episode_logs = []
    ...
    def _get_observation(self):
        ...
        if hasattr(self.reactor, 'get_states'):
            ...
            bacterial_states = [
                self.reactor.microbiome.subpopD[species].count for species in self.reactor.microbiome.subpopD]
    ...
    def step(self, action):
        ...
        if action_enum == KombuchaGymAction.INCREASE:
            self.dilution = min(self.max_dilution,
                                self.dilution + self.unit_change)
        ...
        subpop_counts = {k: v.count for k,
                         v in self.reactor.microbiome.subpopD.items()}
        current_pH = getpH(self.reactor.metabolome.concentration)
        metabolite_conc = dict(
            zip(self.reactor.metabolome.metabolites, self.reactor.metabolome.concentration))
    ...
    def _create_plots(self):
        ...
        if hasattr(self, 'episode_logs') and self.episode_logs:
            ...
            bb_counts = [log['subpop_counts'].get(
                'bb', 0) for log in self.episode_logs]
            ki_counts = [log['subpop_counts'].get(
                'ki', 0) for log in self.episode_logs]
...
def debug_reactor_states():
    ...
    if hasattr(reactor, 'get_states'):
        ...
        print(
            f"get_states() shape: {np.array(states).shape if hasattr(states, '__len__') else 'Not array-like'}")
        print(f"get_states() values: {states}")
    else:
        print("Reactor does not have get_states method!")
        print("Available methods:", [method for method in dir(
            reactor) if not method.startswith('_')])

        # Check metabolome and microbiome
        print(f"\nMetabolome metabolites: {reactor.metabolome.metabolites}")
        print(
            f"Metabolome concentrations shape: {np.array(reactor.metabolome.concentration).shape}")
        print(
            f"Microbiome subpopulations: {list(reactor.microbiome.subpopD.keys())}")

        # Try to construct 12 states manually
        metabolite_states = reactor.metabolome.concentration
        bacterial_states = [
            reactor.microbiome.subpopD[species].count for species in reactor.microbiome.subpopD]
...
def main(**kwargs):
    ...
    if DB is None:
        ...
        print(
            f"Looking for database at: {os.path.join(DATABASEFOLDER, DATABASENAME)}")
        return None

    print(
        f"Parameters: max_steps={param_max_steps}, dilution={param_dilution}, target_ethanol={param_target_ethanol}")
    ...
    try:
        ...
        while not done and step < min(param_max_steps, 20):
            ...
            print(
                f"Step {step+1}: Action={action}, Reward={reward:.4f}, pH={info['pH']:.2f}")
...
def run_direct_reactor_simulation(subpop_dict, media_dict, volume=15):
    ...
    if not ACTUAL_DB_AVAILABLE or DB is None:
        raise RuntimeError(
            "Database not available. Cannot run direct simulation.")
    ...
    for k, v in media_dict.items():
        if k in metabolome.metD:
            ...
            print(
                f"Warning: Metabolite {k} not found in kombucha_media. Skipping.")
    ...
    for k, v in subpop_dict.items():
        if k in microbiome.subpopD:
            ...
            print(
                f"Warning: Subpopulation {k} not found in microbiome. Skipping.")
    ...
    pulse = Pulse(metabolome_feed, microbiome_feed,
                  0, 600, 1000, 0, 0, 0.0, 0.0)
    reactor = Reactor(microbiome, metabolome, [pulse], volume)
